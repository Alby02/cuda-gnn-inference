...
* **GCN:** Uses symmetric normalized aggregation $D^{-1/2}\widehat{A}^T D^{-1/2}$ followed by a dense linear transformation.
* **GraphSAGE:** Implements the mean-aggregator variant. Unlike GCN's symmetric normalization, GraphSAGE performs a distinct two-step update: first, it computes the mean of incoming neighbor features, and second, it concatenates this aggregated neighborhood vector with the node's previous-layer features before applying a dense linear transformation:
  $$h_v^{(k)} = \sigma\left(W \cdot \left[h_v^{(k-1)} \parallel h_{\mathcal{N}(v)}^{(k)}\right]\right)$$
* **Selected parallel mappings:** Destination-owned/vertex-centric for OpenMP, destination/feature 2D mapping for CUDA (see §1.6 and §1.7 below for the rationale).

### 1.2 Graph representation and format 
...
### 1.4 GNN layer parameter generation (model weights) 
...
* **Initialization scheme:** A Glorot/Xavier-style uniform distribution, $\pm\sqrt{6/(f_{in}+f_{out})}$, is used purely to keep magnitudes numerically well-scaled (avoiding overflow/underflow). Because GraphSAGE concatenates the target node feature with the aggregated neighborhood feature, the input dimension to the weight matrix is effectively $2 \times f_{in}$.
...
### 1.6 Software architecture overview 
...
### 1.7 CPU parallelization scheme
...
#### 1.7.2 GCN OpenMP implementation
...
### 1.8 GPU parallelization scheme

#### 1.8.1 Shared strategy rationale 
...
* **No Atomics:** Each thread independently computes the aggregation for its specific feature dimension and writes to a unique memory address, bypassing the need for expensive `atomicAdd` operations in global memory.

#### 1.8.2 GCN CUDA implementation
The GCN aggregation kernel and its run-time-configurable launch geometry are implemented. 
* **Known limitation:** This kernel is not yet integrated into a complete, testable CUDA engine, because it depends on shared CUDA infrastructure not yet available. Cross-executor numerical and performance comparisons currently focus on GraphSAGE.

#### 1.8.3 GraphSAGE CUDA implementation
The CUDA engine maps a 2D grid of threads to the problem: one thread per `(destination node, output feature)` pair. The kernel independently calculates the neighbor mean over the CSC column without atomics, and coordinates with subsequent linear transform kernels using dedicated device buffers.

### 1.9 GraphSAGE Neighbor Sampling (Extension)
An optional, layer-dependent neighbor sampling mechanism is introduced into the GraphSAGE CUDA and CPU aggregation pipelines. By limiting the number of neighbors processed per node during aggregation, it significantly reduces computational overhead and memory bandwidth requirements for highly connected graphs.

* **Layer-Based Decay:** Dynamically calculates the sample size based on network depth to balance performance and information flow:
  * **Layer 0:** Maximum of 25 neighbors.
  * **Deeper Layers:** Decreases linearly (`10 - layer * 2`), with a hard minimum of 5 neighbors.
* **Deterministic Selection:** To avoid overhead and branch divergence introduced by pseudo-random number generation, kernels use a deterministic stride: `edge_idx = e * degree / maxSamples`.
* **Bypass Mechanism:** If a node's degree is $\le$ `maxSamples`, sampling logic is bypassed to process all available neighbors.

### 1.10 Strategies for handling highly skewed degree distributions
